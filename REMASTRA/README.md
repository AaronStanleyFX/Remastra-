...
| ③ AI Stems | Four models. **Demucs v4**: 8 fast stems (vocals, drums, bass, guitar, piano, strings, pads, synths). **MVSep Mega BS-RoFormer**: up to **53 instruments** (lead vocal, backing vocals, kick, snare, hi-hat, toms, violin, viola, cello, trumpet, trombone, saxophone, flute, organ, harp, accordion, sitar…). **BS-RoFormer — Male/female vocals** (aufr33): splits the voice into male and female stems + instrumental residual. **BS-RoFormer-1296** (viperx): high-precision vocals/instrumental split (SDR 12.96 dB). Each stem has mute/solo, a gain control and its own export. |
...
* Publish a release: `git tag v1.5.0 && git push origin v1.5.0`. The GitHub Actions workflow compiles `Remastra.exe` and creates the release with the Windows archive and `RELEASE_NOTES.md`.
