...
def render(audio: np.ndarray, sr: int, cfg: ExportSettings, stems: dict | None = None,
           log=_noop, model_id: str = "mega53") -> np.ndarray:
    ...
    out = spatial.upmix(audio, sr, cfg.layout, use, model_id)
...
def export(path: str, audio: np.ndarray, sr: int, cfg: ExportSettings,
           stems: dict | None = None, log=_noop, model_id: str = "mega53") -> list[str]:
    out = render(audio, sr, cfg, stems, log, model_id)
