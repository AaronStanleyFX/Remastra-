...
# Modèle 0 = Demucs (géré ici) ; modèles suivants = registre BS-RoFormer (mega.REGISTRY)
DEMUCS_LABEL = "Demucs v4 — 8 stems (rapide)"


def roformer_labels() -> list[str]:
    from . import mega
    return [mega.model_label(k) for k in mega.MODEL_IDS]


def model_names() -> list[str]:
    """Libellés du sélecteur de modèle, dans l'ordre : Demucs puis les modèles BS-RoFormer."""
    return [DEMUCS_LABEL] + roformer_labels()


# Rétro-compatibilité (ancienne liste statique) — préférer model_names().
MODELS = [DEMUCS_LABEL, "MVSep Mega BS-RoFormer — 53 stems"]


def stem_label(name: str, model_id: str = "mega53") -> str:
    if name in STEM_LABELS:
        return STEM_LABELS[name]
    from . import mega
    return mega.label(name, model_id)


def stem_color(name: str, model_id: str = "mega53") -> str:
    if name in STEM_COLORS:
        return STEM_COLORS[name]
    from . import mega
    return mega.color(name, model_id)
